...
def tranverse(dict_or_list, path = []):
  ...
  if isinstance(dict_or_list, dict):
    iterator = dict_or_list.keys()
    for k in iterator:
      v = dict_or_list.get(k)
...
def rico():
  ...
  success = 0
  remove = 0
  for root, dirs, files in os.walk('G:\\animations'):
    for name in files:
      ...
      try:
        ...
        for it in item:
          ...
          for key, value in it:
            ...
            if key == 'bounds':
              ...
              boxes.append(value)
          it_list.append([it, key_list, value_list])
        ...
        for it in it_list:
          i = 0
          for idx in range(0, len(it[1])-1):
            key = it[1][idx]
            value = it[2][idx]
            if value == iou_box:
              iou_it = it
        for idx in range(0, len(iou_it[1]) - 1):
          key = iou_it[1][idx]
          value = iou_it[2][idx]
          if key == 'class':
            class_name = value
        ...
        if len(os.listdir(save_dir)) < 4 or len(os.listdir(save_dir)) > 20:
          ...
          path = root.replace('\\animations', '\\split_image').replace('\\gifs', '')
          path = os.path.join(path, 'class_name.txt')
          with open(path, 'a') as label_file:
